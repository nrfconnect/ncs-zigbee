...
/** @file
 *
 * @brief Matter + Zigbee light switch (802.15.4 dispatcher hands radio to Zigbee or Thread).
 */
...
void zigbee_thread_fn()
{
	...
	/*
	 * Block until Matter signals readiness via the first
	 * kCHIPoBLEAdvertisingChange(kActivity_Started) event. By then
	 * dk_buttons_init() has run inside AppTask::Init() and the chained
	 * Zigbee handler can be safely registered.
	 */
