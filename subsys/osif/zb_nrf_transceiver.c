...
zb_bool_t zb_trans_is_receiving(void) {
  zb_bool_t is_receiv =
      (nrf5_data.state == ZB_RADIO_STATE_RECEIVE) ? ZB_TRUE : ZB_FALSE;
...
zb_bool_t zb_trans_is_active(void) {
  zb_bool_t is_active =
      (nrf5_data.state != ZB_RADIO_STATE_SLEEP) ? ZB_TRUE : ZB_FALSE;
...
static void zigbee_nrf_802154_transmitted_raw(
    uint8_t *p_frame, const nrf_802154_transmit_done_metadata_t *p_metadata) {
  ARG_UNUSED(p_frame);
...
static void zigbee_nrf_802154_transmit_failed(
    uint8_t *p_frame, nrf_802154_tx_error_t error,
...
static void zigbee_nrf_802154_tx_ack_started(const uint8_t *p_data) {
  nrf5_data.rx.last_frame_ack_fpb =
      p_data[FRAME_PENDING_OFFSET] & FRAME_PENDING_BIT;
}

static void zigbee_nrf_802154_received_timestamp_raw(uint8_t *p_data,
...
static void zigbee_nrf_802154_receive_failed(nrf_802154_rx_error_t error,
                                             uint32_t id) {
  ARG_UNUSED(id);
  ARG_UNUSED(error);
  nrf5_data.rx.last_frame_ack_fpb = false;
...
static void zigbee_nrf_802154_energy_detected(
...
static void
zigbee_nrf_802154_energy_detection_failed(nrf_802154_ed_error_t error) {
  ARG_UNUSED(error);
...
#if defined(CONFIG_NRF_802154_SER_HOST)
static void
zigbee_nrf_802154_serialization_error(const nrf_802154_ser_err_data_t *err) {
  ARG_UNUSED(err);
...
void nrf_802154_transmitted_raw(
    uint8_t *frame, const nrf_802154_transmit_done_metadata_t *metadata) {
  zigbee_nrf_802154_transmitted_raw(frame, metadata);
}

void nrf_802154_transmit_failed(
    uint8_t *frame, nrf_802154_tx_error_t error,
...
SYS_INIT(zigbee_802154_radio_init, POST_KERNEL,
         CONFIG_ZBOSS_RADIO_INIT_PRIORITY);
