...
When enabled, it selects the following Kconfig options:

* ``CONFIG_DFU_MULTI_IMAGE_SAVE_PROGRESS``
* ``CONFIG_DFU_TARGET_STREAM_SAVE_PROGRESS``
* ``CONFIG_SETTINGS``

In addition, the Zigbee add-on automatically enables a settings backend based on the target SoC: ``CONFIG_NVS`` for non-RRAM SoCs and ``CONFIG_ZMS`` for RRAM SoCs.
...
* The Zigbee FOTA upgrades are currently supported on the nRF52840 DK (PCA10056), nRF5340 DK (PCA10095), nRF54L15 DK (PCA10156), and nRF54LM20 DK (PCA10157).
